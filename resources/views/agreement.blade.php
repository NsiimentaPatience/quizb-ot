...
<body>
    <div class="container mt-5">
        <div class="card">
            ...
            <div class="card-footer">
                <form id="agreementForm" action="{{ route('user.agreement.submit') }}" method="POST">
                    @csrf
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" id="acceptAgreement" name="acceptAgreement" required>
    ...
    <script>
        document.getElementById('agreementForm').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    // If the status is successfully recorded, enable the continue button
                    document.getElementById('continueButton').disabled = false;

                    // Optionally, show a message indicating that the agreement has been accepted
                    alert('User agreement accepted. You may now continue or cancel.');
                } else {
        ...
        document.getElementById('acceptAgreement').addEventListener('change', function() {
            document.getElementById('continueButton').disabled = !this.checked;
        });

        // Handle redirection on Continue button click
